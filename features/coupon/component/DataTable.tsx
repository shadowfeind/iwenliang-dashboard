...
import CreateOrUpdateCoupon from "./CreateOrUpdateCoupon";
import Delete from "@/components/Delete";
import { deleteCoupon } from "../coupon.action";
import { mode } from "@/config/types/mode.types";

export function DataTable({ data }: { data: CouponType[] }) {
  ...
  const [couponId, setCouponId] = React.useState<string | null>(null);
  ...
  const columns: ColumnDef<CouponType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div> {row.getValue("validTill")?.toString()?.slice(0, 15)}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="lowercase">{row.getValue("discountType")}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="lowercase">{row.getValue("discountValue")}</div>
      ),
    },
    {
      id: "actions",
