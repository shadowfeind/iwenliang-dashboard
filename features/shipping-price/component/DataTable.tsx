...
import ViewOrEditShippingPrice from "./ViewOrEditShipingPrice";

export function DataTable({ data }: { data: ShippingPriceType[] }) {
  ...
  const [shippingPrice, setShippingPrice] =
    React.useState<ShippingPriceType | null>(null);

  const handleView = (value: ShippingPriceType, mode: mode) => {
    setMode(mode);
    setModelOpen(true);
    setShippingPrice(value);
  };

  const handleEdit = (value: ShippingPriceType, mode: mode) => {
    setMode(mode);
    setModelOpen(true);
    setShippingPrice(value);
  };

  const columns: ColumnDef<ShippingPriceType>[] = [
    {
      accessorKey: "name",
      header: ({ column }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => <div>{row.getValue("dial_code")}</div>,
    },
    {
      id: "actions",
      cell: ({ row }) => {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <>
                ...
                <DropdownMenuItem
                  onClick={() => handleEdit(row.original, "edit")}
                >
                  Edit
                </DropdownMenuItem>
              </>
